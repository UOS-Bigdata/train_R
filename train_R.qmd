...
# ggplot 으로 그림을 그릴 떄 한글이 깨지지 않게 폰트 Nanum Pen Script를 선택
library(showtext)
...
본 예제에서는 국민체력 100 자료를 이용하여 데이터에 대한 전 처리 및 기초 분석과 시각화을 수행하는 방법을 다룬다.

마지막으로 채력 측정 자료와 등급 지군 자료를 결합하여  보고서에 사용할 수 있는  요약표와 그래프를 만드는 실습을 수행한다. 
...
이 예제에서는 다음과 같은 3 개의 엑셀 화일에 저장된 데이터를 사용할 것이다.

- `sample_data_sport100.xlsx`: 
...
다음은 위에서 설명한 3개의 엑셀화일의 일부를 나타내 그림이다.

::: {#fig-data1}

![`sample_data_sport100.xlsx`](./images/data_1.png){width=80%}
...
![`columns_info.xlsx`](./images/data_2.png){width=80%}
...
더 나아가 별도의 자료  `columns_info.xlsx` 는 변수명 `ITEM_XXX` 에 대한 한글이름, 체력 측정에 대한 부문, 단위, 장상값 범위 등 측정항목에 대한 정보(즉 메타 정보; Meta Infomation)를 포함한다.

이렇게 측정값을 저장하는 데이터과 측정항목의 정보를 저장하는 데이터를 따로 관리하고 서료 결합할 수 있게 하면 자료의 확정성이 커지고 관리가 편리하다.
...
이제 R을 이용하여 위에서 설명한 3 개의 엑셀 화일을 불러오자 . 이를 위해 `readxl` 패키지의 함수 `read_xlsx`를 사용한다.
함수 `read_xlsx` 에서 사용된 `col_names = TRUE` 선택문은 엑셀화일의 첫 번째 행의 내용을  컬럼의 이름으로 하기위한 선택문이다. 
...
이제 불러온 자료의 일부와 구조를 살펴보면서 자료가 정확하게 데이터프레임으로 만들어 졌는지 살펴보자.
...
먼저 데이터프레임 `df_data` 에서  `TEST_MD` 컬럼은 측정 날짜를 나타내는 데이터이지만 엑셀 화일에서 자료 입력시 문자형 변수로 이력이 된다.  일반적으로 날짜를 나타내는 자료는 날짜형 변수로 형식을 바꾸는 것이 다루기 편하다.
...
다음으로 성별을 나타내는 `TEST_SEX` 변수의 값이  1 이면  `남성`, 2 이면 `여성` 으로 문자형으로 바꾸고 범주형 변수(factor)로 변환하자.
...
다음과 같이 `.groups = "drop_last"` 를 이용한 결과를 위의 결과와 비교헤보자. 위에서 처럼 `.groups = "drop_last"` 를 이용하면  `summarise` 함수에서  마지막 그룹화 변수인 `TEST_SEX` 만 제거되고, `TEST_YEAR` 는 여전히 그룹화된 상태로 유지된다. 따라서 결과 데이터프레임은 `TEST_YEAR` 별로 요약된 형태로 나타난다. 반면, `.groups = "drop_last"` 사용하면  그룹화 정보가 모두 제거되어 비율은 전체 자료 기준으로 계산된다.
...
두 형식은 두 함수 `pivot_longer` 와 `pivot_longer` 로 서로 변환할 수 있다.
...
위와 같이 컬럼에 포람된 자료의 내용을 바꾸는 작업은 함수 `case_match` 를 사용할 수도 있다. 예를 들어 위의 요약 통계량을 포함한  데이터프레임에서 통계량의 이름에 대한 컬럼(`stat`)에서 영문 통계량 이름을 다음과 같은 코드로 한글로 바꿀수 있다.
...
이제 상대악력과  윗뭄말아올리기의 관계를 `ggplot` 함수를 이용하여 산점도로 나타내 보자.
...
다음으로 연도별로 BMI(`ITEM_F018`) 분포를 비교해 보자. 이를 위해 `ggplot` 함수를 이용하여 각 연도마다 상자그림으로 분포를 나타낼 수 있다. 먼저 청소냔 남자들의 분포를 비교해 보자.
...
보고서에 사용할 수 있는 표외 그림를 만드는  절차를 설명하려고 한다. 
...
이제 해당 측정항목이 30% 백분수(`Q30`) 미만이면 4등급, 30%-50%(`Q30`-`Q50`) 이면 3등급, 50%-70%(`Q50`-`Q70`) 이면  2등급, 70% 이상(`Q750`)이면 1등급으로 등급 변수 `class` 를 만들어 보자.
...
이제 위에서 구한 등급의 분포를 등급이 변수명이 되는 얿은 형식의 자료로 만들어 보자. 또한 각 년도에 해당하는 총 인원도 계산에서 표에 포함한다.
